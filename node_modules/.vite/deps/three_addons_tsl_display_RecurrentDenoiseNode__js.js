import { Kr as MathUtils, Nn as HalfFloatType, Ps as Vector2, Yr as Matrix4, lo as RenderTarget } from "./three.core-BlZEhazR.js";
import { NodeMaterial, QuadMesh, RendererUtils, TempNode } from "./three_webgpu.js";
import { Discard, EPSILON, Fn, If, Loop, NodeUpdateType, PI, abs, atan, bool, convertToTexture, cos, cross, dot, exp, float, getScreenPosition, getViewPosition, int, log, luminance, mat2, max, mix, nodeObject, normalize, passTexture, property, reflect, sin, smoothstep, sqrt, tan, texture, uniform, unpackRGBToNormal, uv, vec2, vec3, vec4 } from "./three_tsl.js";
import { t as bindAnalyticNoise } from "./RNoise-WDRPAeDv.js";
import { r as ENV_RAY_LENGTH_THRESHOLD } from "./SpecularHelpers-CcCEMm6p.js";
//#region node_modules/three/examples/jsm/tsl/display/RecurrentDenoiseNode.js
var _quadMesh = /*@__PURE__*/ new QuadMesh();
var _size = /*@__PURE__*/ new Vector2();
var _rendererState;
var KERNEL_SAMPLES = 8;
var NOISE_ROTATION_SEED = 83;
var WORLD_RADIUS_SCALE = .1;
var AO_EDGE_STOPPING_BIAS = .05;
var AGGRESSIVITY_RADIUS_MIN = .001;
var DIFFUSE_CHROMA_WEIGHT = 2;
var FLICKER_COV_GATE_MIN = .1;
var FLICKER_COV_GATE_MAX = 2;
/**
* Golden-angle Vogel disk offset.
*
* @tsl
*/
var vogelDisk = Fn(([i, radius]) => {
	const sampleCount = 8;
	const theta = i.add(.5).mul(2.399827721492203);
	const r = radius.mul(sqrt(i.add(.5).div(sampleCount)));
...
* @tsl
*/
var diffuseColorDistance = Fn(([a, b, compressLuma]) => {
	const toYCoCg = (c) => vec3(dot(c, vec3(.25, .5, .25)), c.r.sub(c.b), c.g.sub(c.r.add(c.b).mul(.5)));
	const ya = toYCoCg(a);
	const yb = toYCoCg(b);
...
var _temporalWeight = Fn(([x, strength]) => float(1).div(x.pow(strength))).setLayout({
	name: "temporalWeight",
	type: "float",
	inputs: [{
		name: "x",
...
* @tsl
*/
var getTemporalVarianceFactor = Fn(([frameNum, strength]) => {
	return _temporalWeight(frameNum, strength).max(.05);
}).setLayout({
...
* @tsl
*/
var computeFrustumSize = Fn(([viewZ, tanHalfFovY]) => {
...
* @tsl
*/
var computeHitDistFactor = Fn(([worldRayLength, viewZ, tanHalfFovY]) => {
	const frustumSize = computeFrustumSize(viewZ, tanHalfFovY);
	return worldRayLength.div(frustumSize.max(1e-6)).clamp(0, 1);
}).setLayout({
...
* @tsl
*/
var specularLobeTanHalfAngle = Fn(([roughness, percent]) => {
	return roughness.mul(roughness).mul(sqrt(percent.div(float(1).sub(percent).max(1e-6))));
}).setLayout({
...
var EXP_WEIGHT_SCALE = 4;
var NORMAL_ENCODING_ERROR = 1.5 / 255;
/**
...
* @tsl
*/
var lobeNormalFalloff = Fn(([roughness, aggressivity, invNormalPhi]) => {
	const lobeHalfAngle = max(atan(specularLobeTanHalfAngle(roughness, mix(invNormalPhi.pow2(), float(0), aggressivity.sqrt()).clamp(.1, .99))), float(NORMAL_ENCODING_ERROR));
...
* @tsl
*/
var lobeNormalWeight = Fn(([viewNormal, nNormalV, lobeFalloff]) => {
	return exp(dot(viewNormal, nNormalV).sub(1).mul(lobeFalloff));
}).setLayout({
...
* @tsl
*/
var planeDistance = Fn(([position, nPosition, normal]) => {
...
* @tsl
*/
var karisTemporalBlend = Fn(([denoisedRgb, denoisedRaw, a, flickerSuppression, adaptiveTrust, nbhdMeanLuma, nbhdStddevLuma]) => {
	const localCoV = nbhdStddevLuma.div(nbhdMeanLuma.max(1e-4));
	const trustSuppress = localCoV.mul(adaptiveTrust).mul(a.oneMinus()).clamp(0, .9);
...
*/
var RecurrentDenoiseNode = class extends TempNode {
	static get type() {
	...
	*/
	constructor(inputTexture, camera, options = {}) {
		super("vec4");
		const { depth = null, normal = null, metalRoughness = null, diffuse = null, raw = null, mode = "diffuse", accumulate = true } = options;
		...
		*/
		...
		this.adapt = uniform(.5);
		this.smoothDisocclusions = uniform(true, "bool");
		this.strength = uniform(.25);
	...
	*/
	...
	updateBefore(frame) {
		...
		if (this.camera.isPerspectiveCamera) this._fovY.value = MathUtils.degToRad(this.camera.fov);
		if (frame.frameId !== void 0) this._noiseIndex.value = frame.frameId;
	...
	setup(builder) {
		...
		const sampleDepth = (uvCoord) => this.depthNode?.sample(uvCoord)?.x ?? float(.5);
		...
		const getNeighborhoodStats = Fn(([uvCoord, centerSample]) => {
			...
			const accumulate = (dx, dy, sample) => {
				const neighbor = sample !== void 0 ? sample : texture(this.rawNode, uvCoord.add(vec2(dx, dy).div(this._resolution))).max(0).toConst();
				if (this.alphaSource === "raylength") {
					...
					const w = float(1).div(sampleRl.add(.001));
			...
			accumulate(0, -1);
			accumulate(0, 1);
			return vec4(this.alphaSource === "raylength" ? rlSum.div(rlSumW) : float(1), meanLuma, sqrt(m2Luma.div(lumaCount.max(1))), hasEnvRay.toFloat());
		}).setLayout({
		...
		const denoiseFn = Fn(([uvCoord]) => {
			...
			const runDenoise = () => {
				...
				const aggressivity = getTemporalVarianceFactor(frameNum, this.strength.oneMinus()).oneMinus();
				...
				if (this.alphaSource === "raylength") {
					...
					hasEnvRay.assign(stats.w.greaterThan(.5));
				} else If(this.adaptiveTrust.greaterThan(0), () => {
					const stats = getNeighborhoodStats(uvCoord, raw);
					nbhdMeanLuma.assign(stats.y);
					nbhdStddevLuma.assign(stats.z);
				});
				const tanHalfFovY = this.alphaSource === "raylength" ? tan(this._fovY.mul(.5)).toConst() : null;
				const hitDistFactor = this.alphaSource === "raylength" ? computeHitDistFactor(rl, viewZ, tanHalfFovY).toConst() : float(1);
				...
				If(raw.rgb.length().lessThan(1e-4), () => {
				...
				if (this.mode === "specular") {
					...
					worldRadius.mulAssign(roughness.sqrt().max(.01));
				} else worldRadius.mulAssign(avgAo.pow(2).mul(viewPosition.z.abs()));
				worldRadius.mulAssign(mix(1, AGGRESSIVITY_RADIUS_MIN, aggressivity));
				const T = vec3(0).toVar();
				const B = vec3(0).toVar();
				if (this.mode === "specular") {
					const R = reflect(getSpecularDominantDirection(viewNormal, normalize(viewPosition).negate(), roughness).negate(), viewNormal);
					const Tv = normalize(cross(viewNormal, R));
					const Bv = cross(R, Tv);
					const skewFactor = mix(1, roughness, abs(viewNormal.z).acos().div(float(Math.PI * .5)).clamp(0, 1));
					T.assign(Tv.mul(skewFactor));
					B.assign(Bv);
				} else {
					const Tv = cross(vec3(0, 0, 1), viewNormal).normalize().toVar();
				...
				Loop({
					...
					const skewedDir = mix(sampleDir, polarBias.max(EPSILON).normalize(), this.adapt.mul(aggressivity).mul(polarBias.dot(polarBias).greaterThan(.001).select(1, 0)));
					const offset = rotationMatrix.mul(skewedDir.mul(baseOffset.length().mul(radiusShrink))).toVar();
					const sampleUv = getScreenPosition(viewPosition.add(B.mul(offset.x).add(T.mul(offset.y))), this._cameraProjectionMatrix).toVar();
					...
					const nViewPosition = getViewPosition(sampleUv, sampleDepth(sampleUv), this._cameraProjectionMatrixInverse).toConst();
					...
					if (this.diffuseNode !== null) kernelDiff.addAssign(diffuseColorDistance(centerDiffuse, sampleDiffuse(sampleUv), float(0)).mul(this.diffusePhi).mul(metalness));
					if (this.alphaSource === "ao") {
						...
						const rayLengthFactor = hitDistFactor.sub(neighborHitDistFactor).abs().mul(this.alphaPhi).div(viewPosition.z.abs());
					...
					const nWorldNormal = sampleNormal(sampleUv).transformDirection(this._viewMatrix);
					const depthDiff = planeDistance(viewPosition, nViewPosition, viewNormal).mul(depthWeightScale);
					...
					polarBias.assign(mix(polarBias, sampleDir.mul(w.sub(.5)), .5));
					w.mulAssign(mix(float(1).div(luminance(rawNeighborColor.rgb).pow(2).add(.01)), 1, frameNum.div(5).min(1)));
					...
					If(this.smoothDisocclusions, () => {
						const neighborAWeight = neighborColor.a.greaterThan(texel.a).select(w.mul(.33), 0);
				...
				if (this.accumulate) {
					...
					if (this.rawNode !== null) {
						const blended = karisTemporalBlend(denoised, denoisedRaw, a, this.flickerSuppression, this.adaptiveTrust, nbhdMeanLuma, nbhdStddevLuma);
						result.assign(vec4(blended, a));
					} else {
						const finalDenoised = mix(denoised, denoisedRaw, a);
						result.assign(vec4(finalDenoised, a));
					}
				} else result.assign(vec4(denoised, texel.a));
			};
...
*/
var recurrentDenoise = (inputTexture, camera, options = {}) => nodeObject(new RecurrentDenoiseNode(toTextureNode(inputTexture), camera, options));
//#endregion
export { RecurrentDenoiseNode as default, recurrentDenoise };
