import { Ps as Vector2 } from "./three.core-BlZEhazR.js";
import { TempNode } from "./three_webgpu.js";
import { Break, Fn, If, Loop, NodeUpdateType, abs, bool, clamp, convertToTexture, dot, float, max, min, select, smoothstep, uniform, uniformArray, uv, vec2, vec3 } from "./three_tsl.js";
//#region node_modules/three/examples/jsm/tsl/display/FXAANode.js
/**
...
var FXAANode = class extends TempNode {
	...
	*/
	setup() {
		...
		const _ContrastThreshold = float(.0312);
		const _RelativeThreshold = float(.063);
		const _SubpixelBlending = float(1);
		const Sample = Fn(([uv]) => {
			return textureNode.sample(uv);
		});
		const SampleLuminance = Fn(([uv]) => {
			return dot(Sample(uv).rgb, vec3(.3, .59, .11));
		});
		...
		const SampleLuminanceNeighborhood = (texSize, uv) => {
			...
			const n = SampleLuminanceOffset(texSize, uv, 0, -1);
			const e = SampleLuminanceOffset(texSize, uv, 1, 0);
			const s = SampleLuminanceOffset(texSize, uv, 0, 1);
			const w = SampleLuminanceOffset(texSize, uv, -1, 0);
			const ne = SampleLuminanceOffset(texSize, uv, 1, -1);
			const nw = SampleLuminanceOffset(texSize, uv, -1, -1);
			const se = SampleLuminanceOffset(texSize, uv, 1, 1);
			const sw = SampleLuminanceOffset(texSize, uv, -1, 1);
		...
		const DetermineEdge = (texSize, l) => {
			const horizontal = abs(l.s.add(l.n).sub(l.m.mul(2))).mul(2).add(abs(l.se.add(l.ne).sub(l.e.mul(2))).add(abs(l.sw.add(l.nw).sub(l.w.mul(2)))));
			const vertical = abs(l.e.add(l.w).sub(l.m.mul(2))).mul(2).add(abs(l.se.add(l.sw).sub(l.s.mul(2))).add(abs(l.ne.add(l.nw).sub(l.n.mul(2)))));
		...
		const DetermineEdgeBlendFactor = (texSize, l, e, uv) => {
			...
			If(e.isHorizontal, () => {
				uvEdge.y.addAssign(e.pixelStep.mul(.5));
				edgeStep.assign(vec2(texSize.x, 0));
			}).Else(() => {
				uvEdge.x.addAssign(e.pixelStep.mul(.5));
				edgeStep.assign(vec2(0, texSize.y));
			});
			const edgeLuminance = l.m.add(e.oppositeLuminance).mul(.5);
			const gradientThreshold = e.gradient.mul(.25);
			...
			If(deltaSign.equal(l.m.sub(edgeLuminance).greaterThanEqual(0)), () => {
				...
				blendFactor.assign(float(.5).sub(shortestDistance.div(pDistance.add(nDistance))));
			});
			return blendFactor;
		};
		const ApplyFXAA = Fn(([uv, texSize]) => {
			...
			If(ShouldSkipPixel(luminance).not(), () => {
				...
				const finalBlend = max(pixelBlend, DetermineEdgeBlendFactor(texSize, luminance, edge, uv));
...
* @returns {FXAANode}
*/
var fxaa = (node) => new FXAANode(convertToTexture(node));
//#endregion
export { FXAANode as default, fxaa };
