import { Fs as Vector3, Ht as DepthTexture, Nn as HalfFloatType, Ps as Vector2, Yr as Matrix4, lo as RenderTarget } from "./three.core-BlZEhazR.js";
import { NodeMaterial, NodeUpdateType, QuadMesh, RendererUtils, TempNode } from "./three_webgpu.js";
import { EPSILON, Fn, If, abs, convertToTexture, dFdx, dFdy, dot, exp, float, floor, fwidth, getViewPosition, ivec2, luminance, max, min, mix, nodeObject, normalize, passTexture, screenCoordinate, select, smoothstep, sqrt, struct, texture, textureLoad, uniform, unpackRGBToNormal, uv, vec2, vec3, vec4, velocity } from "./three_tsl.js";
import { n as ENV_RAY_LENGTH, r as ENV_RAY_LENGTH_THRESHOLD } from "./SpecularHelpers-CcCEMm6p.js";
//#region node_modules/three/examples/jsm/tsl/display/TemporalReprojectNode.js
/**
* Maps a resolve (screen) texel to the corresponding beauty-input texel when resolutions differ.
*
* @tsl
*/
var beautyTexelFromScreen = Fn(([screenTexel, beautySize, resolveSize]) => {
...
* @tsl
*/
var projectWorldToUV = Fn(([worldPos, previousViewMatrix, previousProjectionMatrix]) => {
	const resultUV = vec2(-1).toVar();
	const viewSpace = previousViewMatrix.mul(vec4(worldPos, 1));
	...
	If(abs(clipW).greaterThan(float(1e-5)), () => {
		...
		resultUV.assign(ndc.xy.mul(.5).add(.5));
	...
	inputs: [
		...
		{
			name: "previousViewMatrix",
			type: "mat4"
		},
		{
			name: "previousProjectionMatrix",
			type: "mat4"
		}
	]
});
...
* @returns {Node<vec3>}
*/
var rgbToYCoCg = (c) => vec3(dot(c, vec3(.25, .5, .25)), dot(c, vec3(.5, 0, -.5)), dot(c, vec3(-.25, .5, -.25)));
/**
* @param {Node<vec3>} c
* @returns {Node<vec3>}
*/
var ycocgToRGB = (c) => vec3(c.x.add(c.y).sub(c.z), c.x.add(c.z), c.x.sub(c.y).sub(c.z));
var VARIANCE_CLIP_LUMA_SCALE = 10;
/**
...
* @returns {Node<vec3>}
*/
var dampenForVarianceClip = (rgb, flickerSuppression) => {
...
* @tsl
*/
var clipToAABB = Fn(([history, boxMin, boxMax]) => {
	const pClip = boxMax.add(boxMin).mul(.5);
	const eClip = boxMax.sub(boxMin).mul(.5).add(1e-7);
	const vClip = history.sub(pClip);
	const absUnit = vClip.div(eClip).abs();
...
* @tsl
*/
var collectNeighborhood = Fn(([beautyTexture, beautyTexel, inputColor, flickerSuppression]) => {
	const offsets = [
		[-1, -1],
		[-1, 1],
		[1, -1],
	...
	return neighborhoodStruct(mean, stdColor, rayLengthCount.lessThan(.5).select(float(ENV_RAY_LENGTH), rayLengthSum.div(max(rayLengthCount, float(1e-4)))), envProbability, sqrt(m2RayLength.div(max(rayLengthCount, float(1)))).max(.001));
});
/**
...
* @tsl
*/
var applyVarianceClipping = Fn(([historyColor, mean, stdColor, gamma, flickerSuppression]) => {
	...
	return ycocgToRGB(clipToAABB(rgbToYCoCg(historyRGB.div(historyScale)), boxMin, boxMax)).mul(historyScale);
});
var bilinearTapStruct = struct({
	color: "vec4",
	weight: "float",
	confidence: "float"
});
var historyResultStruct = struct({
	color: "vec4",
	tapConfidence: "float",
	minConfidence: "float"
});
...
* @tsl
*/
var sampleBilinearTap = Fn(([historyTexture, previousDepthNode, previousNormalNode, resolution, previousProjectionMatrixInverse, previousCameraWorldMatrix, previousCameraViewMatrix, tapCoord, bilinearWeight, worldPosition, worldNormal]) => {
	const color = textureLoad(historyTexture, tapCoord).max(0);
	const reprojDepth = textureLoad(previousDepthNode, tapCoord).r;
	const reprojViewPos = getViewPosition(vec2(tapCoord).add(.5).div(resolution), reprojDepth, previousProjectionMatrixInverse);
	const reprojWorldPos = previousCameraWorldMatrix.mul(vec4(reprojViewPos, 1)).xyz;
	...
	const normalConfidence = smoothstep(.95, .999, reprojWorldNorm.dot(worldNormal));
	const confidence = smoothstep(0, .01, planeDiff).oneMinus().mul(normalConfidence);
...
function bilinearHistoryTap(ctx, tapOffset, bilinearWeight) {
	return sampleBilinearTap(ctx.historyTexture, ctx.previousDepthNode, ctx.previousNormalNode, ctx.resolution, ctx.previousProjectionMatrixInverse, ctx.previousCameraWorldMatrix, ctx.previousCameraViewMatrix, ctx.reprojICoord.add(tapOffset), bilinearWeight, ctx.worldPosition, ctx.worldNormal);
}
/**
...
* @tsl
*/
var sampleHistory4Tap = Fn(([historyTexture, previousDepthNode, previousNormalNode, resolution, previousProjectionMatrixInverse, previousCameraWorldMatrix, previousCameraViewMatrix, reprojUV, worldPosition, worldNormal, inputColor]) => {
	const reprojPixelCoord = reprojUV.mul(resolution).sub(.5).toVar();
	...
	return historyResultStruct(select(weightSum.greaterThan(.01), colorSum.div(weightSum), vec4(inputColor.rgb, float(1))), maxConf, minConf);
});
/**
...
* @tsl
*/
var reprojectionStretchConfidence = Fn(([historyUV, resolution]) => {
	...
	const disc = fro2.mul(fro2).mul(.25).sub(det.mul(det)).max(0).sqrt();
	return fro2.mul(.5).sub(disc).max(0).sqrt().saturate();
});
/**
...
* @tsl
*/
var reprojectHitPoint = Fn(([rayOrig, rayLength, cameraWorldPosition, previousViewMatrix, previousProjectionMatrix]) => {
	const cameraRay = normalize(rayOrig.sub(cameraWorldPosition)).toVar();
	return projectWorldToUV(rayOrig.add(cameraRay.mul(rayLength)), previousViewMatrix, previousProjectionMatrix);
});
/**
...
* @tsl
*/
var velocityToUVOffset = Fn(([velocity]) => {
	return velocity.mul(vec2(.5, -.5));
}).setLayout({
...
var _rendererState;
var DEFAULT_MAX_VELOCITY_LENGTH = 128;
var VARIANCE_GAMMA_MIN = .5;
var VARIANCE_GAMMA_MAX = 1;
/**
...
*/
var TemporalReprojectNode = class extends TempNode {
	static get type() {
	...
	*/
	constructor(beautyNode, depthNode, normalNode, velocityNode, camera, options = {}) {
		super("vec4");
		const { mode = "diffuse", hitPointReprojection = mode === "specular", accumulate = false } = options;
		if (mode !== "specular" && mode !== "diffuse") throw new Error("TemporalReprojectNode: `mode` must be `diffuse` or `specular`.");
	...
	updateBefore(frame) {
		...
		_quadMesh.render(renderer);
		renderer.setRenderTarget(null);
		if (historySwappedForRestart === true) this._syncHistoryTextureBinding();
		else if (this.accumulate === true) renderer.copyTextureToTexture(this._resolveRenderTarget.texture, this._historyRenderTarget.texture);
	...
	_buildSeed(builder) {
		const seed = Fn(() => {
			const beautyTexel = beautyTexelFromScreen(ivec2(floor(screenCoordinate.xy.sub(.5))), this.beautyNode.size(), this._resolution);
	...
	_buildResolve(builder) {
		...
		return Fn(() => {
			const uvNode = uv();
			const screenTexel = ivec2(floor(screenCoordinate.xy.sub(.5)));
			const depth = textureLoad(this.depthNode, screenTexel).r.toVar();
			depth.greaterThanEqual(1).discard();
			const beautyTexel = beautyTexelFromScreen(screenTexel, this.beautyNode.size(), this._resolution);
			...
			const worldPosition = cameraUniforms.worldMatrix.mul(vec4(viewPosition, 1)).xyz.toVar();
			const sampleHistory = (reprojUV) => sampleHistory4Tap(this._historyTextureNode, this._previousDepthNode, this._previousNormalNode, this._resolution, cameraUniforms.previousProjectionMatrixInverse, cameraUniforms.previousWorldMatrix, cameraUniforms.previousViewMatrix, reprojUV, worldPosition, worldNormal, inputColor.rgb);
			...
			const resolveSpecularHistory = () => {
				const surfValid = historyUV.x.greaterThanEqual(0).and(historyUV.x.lessThanEqual(1)).and(historyUV.y.greaterThanEqual(0)).and(historyUV.y.lessThanEqual(1));
				const historyUV_hit = reprojectHitPoint(worldPosition, neighborhood.get("rayLength"), cameraUniforms.worldPosition, cameraUniforms.previousViewMatrix, cameraUniforms.previousProjectionMatrix).toVar();
				const hitValid = historyUV_hit.x.greaterThanEqual(0).and(historyUV_hit.x.lessThanEqual(1)).and(historyUV_hit.y.greaterThanEqual(0)).and(historyUV_hit.y.lessThanEqual(1)).and(this.hitPointReprojection);
				...
				const wHitRaw = minConfHit.mul(reflectionEdgeFactor).mul(curvatureFactor.oneMinus()).mul(confHit).toConst();
				...
				const color = vec4(hcHit.mul(wHit).add(hcSurf.mul(wSurf)).div(wSum), surf.get("color").a).toVar();
			...
			const clippedRGB = applyVarianceClipping(historyColor, neighborhood.get("mean"), neighborhood.get("stdColor"), varianceGamma, this.flickerSuppression).toVar();
			const clampIntensity = this.clampIntensity.mul(max(motionFactor.mul(10).min(1), .25)).mul(float(1).add(stretchConfidence.oneMinus().add(historyTrust.oneMinus()).clamp()));
			...
			totalConfidence.mulAssign(mix(float(1), historyTrust.mul(.05).add(.95), motionFactor.mul(100).clamp()));
			...
			if (isSpecular) If(inputColor.rgb.length().lessThan(EPSILON), () => {
	...
	_syncHistoryTextureBinding() {
		if (this.accumulate === true || this._externalHistoryTexture === null) this._historyTextureNode.value = this._historyRenderTarget.texture;
		else this._historyTextureNode.value = this._externalHistoryTexture;
	}
	/**
	...
	*/
	setHistoryTexture(source) {
		this._externalHistoryTexture = source && typeof source.getRenderTarget === "function" ? source.getRenderTarget().texture : source;
		this._syncHistoryTextureBinding();
	}
	dispose() {
		this._previousNormalTexture.dispose();
		if (this._previousDepthNode.value !== this._historyRenderTarget.depthTexture) this._previousDepthNode.value.dispose();
		if (this._placeholderPreviousDepthTexture !== this._historyRenderTarget.depthTexture) this._placeholderPreviousDepthTexture.dispose();
...
*/
var temporalReproject = (beautyNode, depthNode, normalNode, velocityNode, camera, options = {}) => nodeObject(new TemporalReprojectNode(convertToTexture(beautyNode), nodeObject(depthNode), nodeObject(normalNode), nodeObject(velocityNode), camera, options));
//#endregion
export { TemporalReprojectNode as default, temporalReproject };
